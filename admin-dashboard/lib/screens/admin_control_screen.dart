...
class _UsersTabState extends State<_UsersTab> {
  ...
  Future<void> _createUser() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error creating user'), backgroundColor: Colors.red));
    }
  }
          ...
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                TextField(controller: editEmail, decoration: const InputDecoration(labelText: 'Email')),
                const SizedBox(height: 12),
                TextField(
                ...
                _roleDropdown(selectedRole, roles, (v) => setState(() => selectedRole = v!)),
              ],
            ),
          ),
          actions: [
              ...
              onPressed: () async {
                try {
                  await ApiService.updateUser(
                    widget.token, user.id,
                    editName.text.trim(), editEmail.text.trim(),
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error updating user'), backgroundColor: Colors.red));
                }
              },
  ...
  Widget _buildUserRow(UserModel user) {
    final roleColors = {'admin': Colors.red, 'manager': Colors.orange, 'viewer': Colors.purple};
    final roleColor = roleColors[user.role] ?? Colors.blue;
    return Card(
      elevation: 1,
      ...
      child: ListTile(
        ...
        trailing: Row(
          ...
          children: [
                ...
                onPressed: () => showDialog(
                  ...
                  builder: (_) => AlertDialog(
                    ...
                    content: Column(
                      ...
                      children: [
                        Text('ID: ${user.id}'), const SizedBox(height: 8),
                        Text('Name: ${user.name}'), const SizedBox(height: 8),
                        Text('Email: ${user.email}'), const SizedBox(height: 8),
            ...
            Tooltip(message: 'Edit', child: IconButton(icon: const Icon(Icons.edit, color: Colors.orange), onPressed: () => _showEditDialog(user))),
            Tooltip(message: 'Delete', child: IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => _showDeleteDialog(user))),
          ],
        ),
      ),
    );
      ...
      builder: (context, constraints) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const Text('User Role', style: TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 8),
                _roleDropdown(_selectedRole, roles, (v) => setState(() => _selectedRole = v!)),
                const SizedBox(height: 16),
                ElevatedButton(
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      IconButton(icon: const Icon(Icons.refresh), onPressed: _loadUsers, tooltip: 'Refresh'),
                    ],
                  ),
                  const SizedBox(height: 12),
                  Expanded(
        ...
        return Padding(
          ...
          child: isWide
              ...
              : Column(children: [formSection, listSection]),
        );
      },
    );
  }
...
Widget _roleDropdown(String value, List<String> roles, ValueChanged<String?> onChanged) {
  return Container(
